...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # isotropy-encouraging loss to prevent elongated, highly anisotropic Gaussians 
        scales = gaussians.get_scaling
        mean_scale = scales.mean(dim=-1, keepdim=True)
        loss_iso = ((scales - mean_scale) ** 2).mean()

        loss += loss_iso * 1.0

        # Depth regularization
        Ll1depth_pure = 0.0
        if False and depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                print(f"Global scale: {gaussians._global_scale}")
